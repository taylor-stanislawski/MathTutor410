...
public class twoDigitNoCarry {
	
	public static int total = 0;
	public static int correct= 0;

	public static void twoDigitNoCarry() {
		int rand = ThreadLocalRandom.current().nextInt(1, 6);
		System.out.println(rand);
		rand = rand * 10;
		int rand1 = ThreadLocalRandom.current().nextInt(1, 6);
		...
		rand2 = rand2 * 10;
		int rand3 = ThreadLocalRandom.current().nextInt(1, 5);
		
		rand2 = rand2 + rand3;
		System.out.println(rand2);
		twoDigitNoCarry re = new twoDigitNoCarry();
		...
		if (rand + rand1 == ans) {
			...
			AbstractAction action = new AbstractAction()
			{
			    ...
			    public void actionPerformed(ActionEvent e)//remove everything because we are done
			    {
					...
					try {
						twoDigitNoCarry();} catch (IllegalArgumentException x) {
						}
			    }
			};
	...
	public void counter(int rand, int rand1) {
		...
		JLabel cLabel = new JLabel("Add two digits (no carry)");
		cLabel.setFont(new Font("Times New Roman", Font.BOLD, 30));
		cLabel.setHorizontalAlignment(SwingConstants.CENTER);
		frame.getContentPane().add(cLabel);
